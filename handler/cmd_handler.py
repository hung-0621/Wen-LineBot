...
class CMD_HANDLER:
    ...
    def send_image(self, url):
        ...
        push_message_request = PushMessageRequest(
            to=self.event.source.group_id,
            messages=[image_message]
        )
        self.line_bot_api.send_push_message(push_message_request)
...
""" + "\n".join([f"- {k}" for k in cmd_dict.keys()])
    ...
    def drink_water(self):
        self.send_image("https://github.com/Wen-Line-Bot/Wen-LineBot/blob/main/images/drink_water.png?raw=true")
    ...
    def __init__(self, event, line_bot_api):
        ...
        self.cmd_dict["嗨張子儀"] = lambda:self.send_message("嗨張子儀，今天的張子儀也很張子儀，今天義大利麵也要記得拌42號混凝土ㄛ")
        self.cmd_dict["欸張子儀"] = lambda:self.send_message("欸張子儀，"+self.get_response_from_url())
        ...
        self.cmd_dict["菜就多練"] = lambda:self.send_image("菜就多練，輸不起就別玩，以前是以前，現在是現在，哥們你老是拿以前比那你怎麼不拿你剛出生比")
        self.cmd_dict["我個人認為"] = lambda:self.send_image("我個人認為義大利麵就應該拌42號混凝土，因為這個螺絲釘的長度很容易直接影響到挖掘機的扭矩。你往裡砸的時候，一瞬間他就會產生大量的高蛋白，俗稱UFO，會嚴重影響經濟的發展，以至於對整個太平洋，和充電器的核污染。再者說透過勾股定理很容易推斷出人工飼養的東條英機，他是可以捕獲野生的三角函數，所以說不管這秦始皇的切面是否具有放射性，特朗普的N次方是否含有沈澱物，都不影響到沃爾瑪跟維爾康在南極會合。")
        self.cmd_dict["你說的對"] = lambda:self.send_message("你說得對，但是烏拉圭的人口有345.7萬，同時，僅澳大利亞就有4700萬隻袋鼠。如果袋鼠決定入侵烏拉圭，那麼每一個烏拉圭人都要打14隻袋鼠，你不知道，你不在乎，你只關心你自己。")
        self.cmd_dict["沒錯只有你"] = lambda:self.send_message("沒錯只有你，你是智慧的結晶，你是文明的瑰寶，你是人類的獨苗，你是上帝的遺珠，你是雨後的彩虹，你是夤夜的皎月，你是晨昏的彩霞，你是銀河的唯一，你是樹枝的芽杈，你是滄海的珊瑚，你是最後的希望，你是問題的正解，你是莫蒂的瑞克，你是法國的拿破崙，你是歷史的書寫者，你是人類的光明，你是電你是光你是唯一的神話")
        self.cmd_dict["padoru"] = lambda:self.send_message("hasi re so ri yo\nkaze no you ni\ntsuki mi hara wo\nPADORU！PADORU！")
