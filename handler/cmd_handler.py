...
class CMD_HANDLER:
    ...
    def __init__(self, event, line_bot_api, line_helper):
        ...
        self.cmd_dict["嗨張子儀"] = lambda: self.line_helper.send_message(
            "嗨張子儀，今天的張子儀也很張子儀，今天義大利麵也要記得拌42號混凝土ㄛ")
        self.cmd_dict["欸張子儀"] = lambda: self.line_helper.send_message(
            "欸張子儀，"+my_func.get_response_text_from_url(vars_consts.RANDOM_SENTENCE_URL))
        ...
        self.cmd_dict["我個人認為"] = lambda: self.line_helper.send_message(
        ...
        self.cmd_dict["菜就多練"] = lambda: self.line_helper.send_message(
            "菜就多練，輸不起就別玩，以前是以前，現在是現在，哥們你老是拿以前比那你怎麼不拿你剛出生比")
        self.cmd_dict["你說的對"] = lambda: self.line_helper.send_message(
            "你說的對，但是烏拉圭的人口有345.7萬，同時，僅澳大利亞就有4700萬隻袋鼠。如果袋鼠決定入侵烏拉圭，那麼每一個烏拉圭人都要打14隻袋鼠，你不知道，你不在乎，你只關心你自己。")
        self.cmd_dict["沒錯只有你"] = lambda: self.line_helper.send_message(
        ...
        self.cmd_dict["躺著喝水水"] = lambda: self.line_helper.send_image_with_msg(
            url="https://raw.githubusercontent.com/Wen-Line-Bot/Wen-LineBot/main/images/drink_water_flip.jpg", msg="水量++\n(流速 Max)")
        self.cmd_dict["親情牌"] = lambda: self.line_helper.send_message(
            "我趕你馬")
        ...
        self.cmd_dict["青椒"] = lambda: self.line_helper.send_image_with_msg(
            url="https://raw.githubusercontent.com/hung-0621/Wen-LineBot/hung_edit/images/Green_pepper.jpg",msg="@周青椒")
...
""" + "\n".join([f"- {k}" for k in self.cmd_dict.keys()])
        ...
        self.cmd_dict["bot help"] = lambda: self.line_helper.send_message(
            help_msg)
