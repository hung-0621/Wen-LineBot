...
class CMD_HANDLER:
    ...
    # Text commands
    TEXT_CMD: Dict[str, str] = {
        "嗨張子儀": "嗨張子儀，今天的張子儀也很張子儀，今天義大利麵也要記得拌42號混凝土ㄛ",
        "我個人認為": "我個人認為義大利麵就應該拌42號混凝土，因為這個螺絲釘的長度很容易直接影響到挖掘機的扭矩。你往裡砸的時候，一瞬間他就會產生大量的高蛋白，俗稱UFO，會嚴重影響經濟的發展，以至於對整個太平洋，和充電器的核污染。再者說透過勾股定理很容易推斷出人工飼養的東條英機，他是可以捕獲野生的三角函數，所以說不管這秦始皇的切面是否具有放射性，特朗普的N次方是否含有沈澱物，都不影響到沃爾瑪跟維爾康在南極會合。",
        ...
        "菜就多練": "菜就多練，輸不起就別玩，以前是以前，現在是現在，哥們你老是拿以前比那你怎麼不拿你剛出生比",
        "你說的對": "你說的對，但是烏拉圭的人口有345.7萬，同時，僅澳大利亞就有4700萬隻袋鼠。如果袋鼠決定入侵烏拉圭，那麼每一個烏拉圭人都要打14隻袋鼠，你不知道，你不在乎，你只關心你自己。",
        "沒錯只有你": "沒錯只有你，你是智慧的結晶，你是文明的瑰寶，你是人類的獨苗，你是上帝的遺珠，你是雨後的彩虹，你是夤夜的皎月，你是晨昏的彩霞，你是銀河的唯一，你是樹枝的芽杈，你是滄海的珊瑚，你是最後的希望，你是問題的正解，你是莫蒂的瑞克，你是法國的拿破崙，你是歷史的書寫者，你是人類的光明，你是電你是光你是唯一的神話",
        "padoru": "hasi re so ri yo\nkaze no you ni\ntsuki mi hara wo\nPADORU！PADORU！",
        "親情牌": "我趕你馬",
        "不然投票": "我投張子儀一票",
        "我不同意": "我也不同意",
    }
    ...
    # Image commands
    # format -> cmd: (image_url, msg)
    IMAGE_MSG_CMD: Dict[str, tuple] = {
        "喝水水": (
            "https://raw.githubusercontent.com/Wen-Line-Bot/Wen-LineBot/main/images/drink_water.jpg",
            "水量++",
        ),
        "躺著喝水水": (
            "https://raw.githubusercontent.com/Wen-Line-Bot/Wen-LineBot/main/images/drink_water_flip.jpg",
            "水量++\n(流速 Max)",
        ),
        "啟動": (
            "https://media.9game.cn/gamebase/ieu-gdc-pre-process/images/20231012/7/23/a46362917681efe17e936cd468be76fc.jpg",
            "原神啟動",
        ),
        "青椒": (
            "https://raw.githubusercontent.com/Wen-Line-Bot/Wen-LineBot/main/images/green_pepper.png",
            "@周青椒",
        ),
    }
    ...
    def hey_zzy_helper(self) -> str:
        base_sentence = "欸張子儀，"
        random_sentence = my_func.get_response_text_from_url(
            vars_consts.RANDOM_SENTENCE_URL
        )
        return base_sentence + random_sentence

    # 張子儀不會 + random sentence
    def zzy_can_not_helper(self) -> str:
        pool = [
            "李多慧",
            "茶湯會",
            "獅子會",
        ...
        base_sentence = "張子儀不會，可是"
        random_sentence = random.choice(pool)
        return base_sentence + random_sentence
    ...
    def help_message_helper(self) -> str:
        all_cmds = [k for k in self.cmd_dict.keys() if k != "bot help"]
